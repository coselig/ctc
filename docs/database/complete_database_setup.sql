...
DROP POLICY IF EXISTS "Allow authenticated users to manage permissions" ON public.floor_plan_permissions;

DROP POLICY IF EXISTS "Users can view all photo records for accessible floor plans" ON public.photo_records;
DROP POLICY IF EXISTS "Users can insert photo records for accessible floor plans" ON public.photo_records;
DROP POLICY IF EXISTS "Users can update their own photo records" ON public.photo_records;
DROP POLICY IF EXISTS "Users can delete photo records based on permissions" ON public.photo_records;

-- 刪除舊的視圖
DROP VIEW IF EXISTS user_permissions_summary;
DROP VIEW IF EXISTS floor_plan_stats;
...
-- 刪除舊的索引
DROP INDEX IF EXISTS idx_floor_plan_permissions_url;
DROP INDEX IF EXISTS idx_floor_plan_permissions_user;
DROP INDEX IF EXISTS idx_floor_plan_permissions_owner;
DROP INDEX IF EXISTS idx_photo_records_floor_plan;
DROP INDEX IF EXISTS idx_photo_records_floor_plan_path;
DROP INDEX IF EXISTS idx_photo_records_user_id;

-- 刪除舊的表格
DROP TABLE IF EXISTS public.photo_records CASCADE;
DROP TABLE IF EXISTS public.floor_plan_permissions CASCADE;
...
-- 創建索引以提升查詢效能
CREATE INDEX idx_floor_plan_permissions_url ON public.floor_plan_permissions(floor_plan_url);
CREATE INDEX idx_floor_plan_permissions_user ON public.floor_plan_permissions(user_id);
CREATE INDEX idx_floor_plan_permissions_owner ON public.floor_plan_permissions(floor_plan_url, is_owner);

-- 啟用 floor_plan_permissions 表的 RLS
...
CREATE TABLE public.photo_records (
    id uuid DEFAULT gen_random_uuid() PRIMARY KEY,
    floor_plan_path text NOT NULL,  -- 對應 Flutter 中的 floorPlanPath
    floor_plan_url text,            -- 為了與權限系統相容（可選）
    x_coordinate double precision NOT NULL,  -- 對應 Flutter 中的 point.dx
    y_coordinate double precision NOT NULL,  -- 對應 Flutter 中的 point.dy
    image_url text NOT NULL,        -- 對應 Flutter 中的 imagePath
    description text,               -- 對應 Flutter 中的 description
    user_id text NOT NULL,          -- 對應 Flutter 中的 userId
    username text NOT NULL,         -- 對應 Flutter 中的 username
    created_at timestamp with time zone DEFAULT timezone('utc'::text, now()) NOT NULL,
    updated_at timestamp with time zone DEFAULT timezone('utc'::text, now()) NOT NULL
);

-- 創建索引
CREATE INDEX idx_photo_records_floor_plan ON public.photo_records(floor_plan_url);
CREATE INDEX idx_photo_records_floor_plan_path ON public.photo_records(floor_plan_path);
CREATE INDEX idx_photo_records_user_id ON public.photo_records(user_id);

-- 啟用 photo_records 表的 RLS
ALTER TABLE public.photo_records ENABLE ROW LEVEL SECURITY;

-- photo_records 表的 RLS 策略
-- 所有有權限查看設計圖的用戶都能看到該設計圖上的所有座標點
CREATE POLICY "Users can view all photo records for accessible floor plans" ON public.photo_records
    FOR SELECT TO authenticated USING (
        EXISTS (
            ...
            AND user_id = auth.uid()
        ) OR photo_records.floor_plan_url IS NULL  -- 允許沒有設定 floor_plan_url 的記錄
    );
...
CREATE POLICY "Users can insert photo records for accessible floor plans" ON public.photo_records
    FOR INSERT TO authenticated WITH CHECK (
        EXISTS (
            ...
            AND user_id = auth.uid()
            AND permission_level >= 1
        ) OR photo_records.floor_plan_url IS NULL  -- 允許插入沒有設定 floor_plan_url 的記錄
    );
