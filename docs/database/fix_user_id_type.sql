-- 修復 add_user_permission RPC 函數中的 user_id 類型匹配問題
...
BEGIN
    ...
    IF EXISTS(
        SELECT 1 FROM public.floor_plan_permissions fp
        WHERE fp.floor_plan_url = p_floor_plan_url 
        AND fp.user_id = p_target_user_id::text
    ) THEN
        RETURN json_build_object('success', false, 'error', '用戶已有此設計圖的權限');
    END IF;
    
    -- 插入新權限記錄 (user_id 儲存為 text)
    INSERT INTO public.floor_plan_permissions (
        ...
        p_floor_plan_url,
        p_floor_plan_name,
        p_target_user_id::text,  -- 轉換為 text 類型
        p_target_user_email,
        p_permission_level,
        false,
        NOW(),
